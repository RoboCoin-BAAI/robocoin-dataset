...
def _get_video_labels_dict(source_annotation_data: dict) -> dict:
    result = []
    for item in source_annotation_data:
        epside_annotation = []
        for label in item.get("videoLabels", []):
            for idx in range(len(label["ranges"])):
                epside_annotation.append(  # noqa: PERF401
                    {
        ...
        result.append(epside_annotation)
    return result


# def _check_label_range(source_annotation_data: dict) -> list:
#     error_result = []
#     episode_annotation
#     for episode_annotation in source_annotation_data:
#         frame_start_idx = []
#         frame_end_idx = []
#         for label in episode_annotation.get("videoLabels", []):
#             for idx in range(len(label["ranges"])):
#                 frame_start_idx.append(label["ranges"][idx]["start"])
#                 frame_end_idx.append(label["ranges"][idx]["end"])
#         epis


# def _get_start_end_frame_idx(source_annotation_data: dict) -> list[dict]:
#     result = []
#     for


# Step 1: 读取原始 JSON，提取所有 timelinelabels 的中文标签
def _extract_labels(episode_annotations: dict) -> list[str]:
    labels_set = set()
    for episode_annotation in episode_annotations:
        for annotation in episode_annotation:
            labels_set.add(annotation.get(SUB_TASK))
...
def _process_batch_labels(label_list: list[str], ds_api_key: str) -> list[str]:
    """
    ...
    for i, label in enumerate(label_list):
        lang = _detect_language(label)
        if lang == "zh":
            zh_labels.append((i, label))
        elif lang == "en":
            en_labels.append((i, label))
        else:
    ...
    if zh_labels:
        zh_text = "\n".join(f"{idx + 1}. {cmd}" for idx, (orig_idx, cmd) in enumerate(zh_labels))
    ...
    if en_labels:
        en_text = "\n".join(f"{idx + 1}. {cmd}" for idx, (orig_idx, cmd) in enumerate(en_labels))
    ...
    if not prompt_parts:
        return label_list  # 如果没有需要处理的指令，直接返回
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 验证结果数量
            expected_count = len(zh_labels) + len(en_labels)
            if len(cleaned_results) != expected_count:
                raise ValueError(
                    f"处理结果数量({len(cleaned_results)})与预期数量({expected_count})不匹配"
                )
            final_results = [""] * len(label_list)

            # 填充中文翻译结果
            for (orig_idx, _), result in zip(zh_labels, cleaned_results[: len(zh_labels)]):
                final_results[orig_idx] = result

            # 填充英文优化结果
            en_start_idx = len(zh_labels)
            for (orig_idx, _), result in zip(
                en_labels, cleaned_results[en_start_idx : en_start_idx + len(en_labels)]
            ):
                final_results[orig_idx] = result
            ...
            for i in range(len(label_list)):
                if not final_results[i]:
                    final_results[i] = label_list[i]  # 保持原样
...
def _transform_labels(original_annotations: list[list[dict]], mapping: dict) -> list[str]:
    new_annotations = original_annotations.copy()
    for episode_annotation in new_annotations:
        for item in episode_annotation:
            if SUB_TASK in episode_annotation:
                item[SUB_TASK] = mapping.get(item[SUB_TASK], "")
    return new_annotations


def _modify_frame_idx(annotations: list[list[dict]]) -> dict:
    modifed_annotations = annotations.copy()
    for episode_annotation in modifed_annotations:
        for item in episode_annotation:
            item[START_FRAME_IDX] = item[START_FRAME_IDX] - 1
            item[END_FRAME_IDX] = item[END_FRAME_IDX] - 1

    return modifed_annotations
    ...
    def _sync_process_task(self, task_content: dict) -> dict:
        try:
            ...
            with open(subtask_annotation_source_file_path) as f:
                ...
                en_labels = _process_batch_labels(labels, ds_api_key=ds_api_key)
